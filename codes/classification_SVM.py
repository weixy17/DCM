...
with open('test_idx.txt','r') as f:
    for count in range (4):
        ...
        model_list=test_findfile('./weights/','.pickle',run_id+'_'+str(para1)+'_'+str(para2)+'_'+str(para3)+'_'+str(count))
        fmodel = open(model_list[-1],'rb')
        model = pickle.load(fmodel)
        fmodel.close()
        pred=model.predict(X_test)
        true=dataset['test_label']
...
print('C={},n_estimators={},lr={}'.format(para1,para2,para3))
